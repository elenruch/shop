...
@section('content')
    <div class="row base_row">
        <div class="panel panel-default bas">
            ...
            <form class="form-horizontal" role="form" method="POST" action="{{ url('/basket/order') }}">
                {!! csrf_field() !!}
                ...
                @foreach($tov as $one => $value)
                    @if(isset($value->id))
                        ...
                        <tr>
                            <td>
                                <div align="center">
                                    <img src="{{asset((isset($value->picture)) ? '/uploads/thumb/'.$value->picture : '#')}}"
                                         align="center"/>
                                </div>
                            </td>
                            <td>
                                ...
                                <a href="{{asset('basket/delete/'.$value->id)}}"
                                   class="btn-times btn-cart">&times</a>
                                    ...
                                    <table width="100%">
                                        <tr>
                                            <td>
                                                <input type="number" min="0" name="{{$value->id}}" class="wid"
                                                       value="{{$colvo}}">khjk
                                            </td>
                                        </tr>
                                    </table>
